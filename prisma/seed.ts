...
async function main() {
  ...
  const words = [
    ...
    { id: 'seed-khashar',  term: 'خاشار',   pronunciation: 'khashar',  meaningEn: 'Dry twigs and brush gathered as kindling; also used figuratively for potential', meaningFa: 'شاخه‌های خشک جمع‌شده برای آتش‌افروزی؛ استعاری برای استعداد پنهان', exampleEn: 'She gathered khashar before the golmeh began.',        exampleFa: 'قبل از شروع گلمه، خاشار جمع کرد.',                         photoUrl: PIC('dry-twigs', 600, 360) },
    { id: 'seed-vasht',    term: 'وَشت',    pronunciation: 'vasht',    meaningEn: 'The piercing cold of the hour just before dawn in the desert',           meaningFa: 'سرمای تیز ساعتی پیش از سپیده‌دم در بیابان',              exampleEn: 'Only the brave face the vasht alone.',                  exampleFa: 'فقط شجاع‌ها تنها با وشت روبرو می‌شوند.',                     photoUrl: PIC('cold-dawn', 600, 360) },
    { id: 'seed-nareh',    term: 'نارِه',   pronunciation: 'nareh',    meaningEn: 'A faint echo or whisper, especially sounds traveling through qanat tunnels', meaningFa: 'صدای کمرنگ یا زمزمه، به‌ویژه صداهایی که از دل قنات می‌آیند', exampleEn: 'We heard a nareh from deep inside the qanat.',           exampleFa: 'نارِه‌ای از اعماق قنات شنیدیم.',                            photoUrl: PIC('tunnel-echo', 600, 360) },
  ];
  ...
  const proverbs = [
    ...
    { id: 'seed-proverb-5', textEn: 'No secret survives the golmeh.', textFa: 'هیچ رازی از گلمه جان سالم به در نمی‌برد.', meaningEn: 'In communal evening gatherings, everything is eventually shared.', meaningFa: 'در گردهمایی‌های شبانه، در نهایت همه چیز به اشتراک گذاشته می‌شود.', usageEn: 'Said when gossip or news spreads quickly through the community.', usageFa: 'وقتی شایعه یا خبری سریع در جامعه پخش می‌شود.' },
    { id: 'seed-proverb-6', textEn: 'First the larg, then the havarak — nature prepares you before it tests you.', textFa: 'اول لرگ می‌آید، بعد هورک — طبیعت قبل از آزمون آماده‌ات می‌کند.', meaningEn: 'A warning comes before every hardship, if you know how to read it.', meaningFa: 'پیش از هر سختی هشداری می‌آید، اگر بلد باشی بخوانیش.', usageEn: 'Encourages attentiveness to early signs of change.', usageFa: 'برای تشویق به توجه به نشانه‌های اولیه تغییر.' },
    { id: 'seed-proverb-7', textEn: 'The pile will guide you if you trust it.', textFa: 'پیله راهت را نشان می‌دهد اگر بهش اعتماد کنی.', meaningEn: 'Soft or subtle guidance is still guidance — trust the gentle signs.', meaningFa: 'راهنمایی نرم هم راهنمایی است — به نشانه‌های ملایم اعتماد کن.', usageEn: 'Said to someone who ignores quiet guidance in favor of dramatic signs.', usageFa: 'به کسی که راهنمایی آرام را نادیده می‌گیرد.' },
    { id: 'seed-proverb-8', textEn: 'The kashkam knows the way even when the eyes do not.', textFa: 'کشکم راه را می‌داند حتی وقتی چشم‌ها نمی‌بینند.', meaningEn: 'Trust your other senses; sound and instinct can guide you through darkness.', meaningFa: 'به حواس دیگرت اعتماد کن؛ صدا و غریزه در تاریکی راهت می‌دهند.', usageEn: 'Said to someone who feels lost but should trust their instincts.', usageFa: 'به کسی که گم شده اما باید به غریزه‌اش اعتماد کند.' },
  ];
